    ...
    <section v-if="modo === 'control'" class="space-y-4">
      <div class="grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div class="card">
          ...
          <div v-if="controlNumero.reservas_activas?.length" class="mt-4 border-t border-slate-200 pt-3 dark:border-slate-700"><p class="input-label">Números apartados por trabajos en curso</p><div class="mt-2 flex flex-wrap gap-2"><button v-for="reserva in controlNumero.reservas_activas" :key="reserva.numero" type="button" class="rounded-md border border-sky-200 bg-sky-50 px-3 py-2 text-left text-xs hover:border-sky-500 dark:border-cyan-500/20 dark:bg-cyan-500/10" @click="abrirReserva(reserva.registros[0])"><strong class="font-mono text-sky-800 dark:text-cyan-200">{{ String(reserva.numero).padStart(4, '0') }}</strong><span class="ml-2 text-slate-600 dark:text-slate-300">{{ reserva.registros[0]?.estudiante }}</span><small class="mt-1 block text-slate-500">{{ etiquetaEstado(reserva.registros[0]?.estado) }} · abrir trabajo</small></button></div></div>
        </div>
        ...
        <div v-if="subvista === 'consulta' && ['en_elaboracion_secretaria', 'consulta_previa'].includes(seleccionado.estado)" class="mt-5 border-t border-slate-200 pt-5 dark:border-slate-700">
          ...
          <div v-if="seleccionado.consultas?.length" class="mt-3 space-y-2">
            <div v-for="item in seleccionado.consultas" :key="item.uuid" class="flex flex-wrap items-center justify-between gap-2 rounded-md bg-slate-100 px-3 py-2 dark:bg-slate-800/60">
              <div><p class="text-xs font-medium text-slate-900 dark:text-white">{{ item.docente }} · {{ item.tipo_participacion }}</p><p class="text-[11px] text-slate-500 dark:text-slate-400">{{ item.correos_destino?.join(', ') || 'Sin correo registrado' }} · {{ item.cantidad_accesos ? `abrió ${item.cantidad_accesos} vez/veces` : 'enlace aún no abierto' }} · vence {{ fechaCorta(item.fecha_expiracion) }}</p><p v-if="item.nota_respuesta" class="text-[11px] text-slate-500 dark:text-slate-400">{{ item.nota_respuesta }}</p></div>
          ...
          <div v-if="enlacesGenerados.length" class="mt-3 rounded-md border border-sky-200 bg-sky-50 p-3 dark:border-cyan-500/30 dark:bg-cyan-950/20">
            <p class="text-xs font-semibold text-sky-900 dark:text-cyan-200">Enlaces recién generados</p><p class="mt-1 text-[11px] text-sky-800 dark:text-cyan-100">El sistema prepara y registra el enlace. El correo todavía se envía manualmente hasta configurar una cuenta institucional de salida.</p>
...
async function abrirReserva(registro) { if (!registro?.uuid) return; modo.value = 'trabajo'; await seleccionar({ uuid: registro.uuid }) }
